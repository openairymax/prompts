...
class TestDetectHallucination:
    ...
    def test_value_mismatch_non_numeric(self):
        # _detect_hallucination only flags numeric deviations, not string mismatches
        assert _detect_hallucination({"a": "hello"}, {"a": "world"}) is False
...
class TestPairedTTest:
    ...
    def test_clear_improvement(self):
        import random
        random.seed(42)
        baseline = [1.0 + random.uniform(-0.05, 0.05) for _ in range(30)]
        candidate = [2.0 + random.uniform(-0.05, 0.05) for _ in range(30)]
        result = _paired_t_test(baseline, candidate)
